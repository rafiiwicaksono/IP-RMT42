...
describe(`/foods/admin/:id`, () => {
    test(`success delete food with role admin (200)`, async () => {
        const food = await Food.findByPk(foodId)
        let { status, body } = await request(app)
            .delete(`/foods/admin/${foodId}`)
            .set(`Authorization`, `Bearer ${tokenAdmin}`)
        expect(status).toBe(200)
        expect(body).toBeInstanceOf(Object)
        expect(body).toHaveProperty("message", `${food.name} success to delete`)
    })
    ...
    test(`Food not found (404)`, async () => {
        let { status, body } = await request(app)
            .delete(`/foods/admin/${invalidFoodId}`)
            .set(`Authorization`, `Bearer ${tokenAdmin}`)
    ...
    test(`forbidden delete data food (403)`, async () => {
        let { status, body } = await request(app)
            .delete(`/foods/admin/${foodId}`)
